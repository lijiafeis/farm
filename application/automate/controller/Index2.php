...
class Index2 extends Controller{
    ...
    public function index(){
        $userWithInfo  = db('user_withdraw_log') -> field('id,state,gold,user_id,type,name,alipay_number,create_time') -> where(['state' => 3]) -> order('id asc') -> limit(120,150) ->  select();
        if(!$userWithInfo){return;}
//        file_put_contents('/home/c.txt',3,FILE_APPEND);
